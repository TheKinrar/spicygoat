...
int32_t PacketData::readInt() {
    ...
    auto val = boost::endian::endian_load<int32_t, sizeof(int32_t), boost::endian::order::big>((const uint8_t*) data + pos);
    pos += 4;
    return val;
}
...
uint32_t PacketData::readUnsignedInt() {
    auto val = boost::endian::endian_load<uint32_t, sizeof(uint32_t), boost::endian::order::big>((const uint8_t*) data + pos);
    pos += 4;
    return val;
}
...
int64_t PacketData::readLong() {
    auto val = boost::endian::endian_load<int64_t, sizeof(int64_t), boost::endian::order::big>((const uint8_t*) data + pos);
    pos += 8;
    return val;
}
...
uint64_t PacketData::readUnsignedLong() {
    auto val = boost::endian::endian_load<uint64_t, sizeof(uint64_t), boost::endian::order::big>((const uint8_t*) data + pos);
    pos += 8;
    return val;
}
...
void PacketData::writeUuid(const boost::uuids::uuid& uuid, std::vector<std::byte>& bytes) {
